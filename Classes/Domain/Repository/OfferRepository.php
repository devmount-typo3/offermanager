  ...
  public function findAllOffers($limit, $cuid=-1) {
    ...
    if ($cuid >= 0) {
      $result_repeated_temp = $GLOBALS['TYPO3_DB']->exec_SELECTgetRows(
        ...
        'tx_cal_event e LEFT JOIN tx_cal_event_category_mm m ON e.uid = m.uid_local LEFT JOIN tx_cal_category c ON c.uid = m.uid_foreign',
        'c.uid = ' . intval($cuid) . ' AND e.hidden = 0 AND e.deleted = 0 AND (e.freq NOT LIKE "none" OR e.rdate IS NOT NULL) AND e.calendar_id <> ' . intval($nocalender),
        '',
        'e.start_date ASC'
      );
    }
    // display all events
      ...
      $result_repeated_temp = $GLOBALS['TYPO3_DB']->exec_SELECTgetRows(
        'uid,start_date,end_date,start_time,end_time,title,teaser,description,dates_description,freq,until,cnt,intrval,rdate',
        'tx_cal_event',
        'hidden = 0 AND deleted = 0 AND (freq NOT LIKE "none" OR rdate IS NOT NULL) AND calendar_id <> ' . intval($nocalender),
        '',
        'start_date ASC',
        ($limit > 0) ? $limit : ''
      );
    ...
    foreach ($result_repeated_temp as $key => $event) {
      if ($event['until'] > 0 and $event['until'] < $date) {
        continue;
      }
      $nextdate = '';
      // check freq
